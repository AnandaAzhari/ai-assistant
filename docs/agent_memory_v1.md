...
## Status
- v0.2. Working Memory dan Long-Term Feedback Memory untuk Finance Agent dan Document Agent sudah ada implementasinya. Untuk Social Media Agent, modul memory-nya **sudah diimplementasikan** mengikuti pola yang sama:
  - Working Memory: `app/content_session.py` (`ContentSessionStore`) — state satu draft konten yang aktif, berkunci `scope_id` "usaha:platform:content_id" via `ContentSessionStore.build_scope()`, payload terstruktur ber-`version`, bisa direset dengan `clear()` setelah konten selesai (published/retracted).
  - Long-Term Feedback Memory: `app/content_learning.py` (`ContentLearningStore`) — dua log append-only: `content_feedback_log` (koreksi/persetujuan owner terhadap draft, dibaca lewat `recent_feedback()`/`recurring_feedback_patterns()`, hanya pola yang berulang ≥2 kali yang dianggap sinyal, mengikuti Content Learning aturan 3-4 di `agents/social_media_agent.md`) dan `content_performance_log` (metrik setelah tayang, dibaca lewat `top_performing_topics()` sebagai sinyal ide berikutnya, Content Learning aturan 5). Kedua log selalu difilter per `business` (aturan 6 — tidak pernah tercampur antar usaha). `has_sufficient_history()` membantu membedakan cold-start (aturan 8) vs riwayat internal sudah cukup.
  - Belum ada: Content Studio/Content Pipeline itu sendiri (draft generation, scheduling, publishing) yang akan memanggil kedua store ini — itu langkah implementasi berikutnya untuk Social Media Agent. Reference/Static Knowledge (Brand Profile) sudah ada duluan di `brand_profiles/*.md`.
