...
- [ ] **Belum**: kill switch (mematikan auto-send per channel/seluruh sistem) — didokumentasikan di `policies/security_policy.md`/`policies/approval_policy.md` tapi belum ada implementasi kode. Disarankan dikerjakan sebelum nomor WA bisnis live ke pelanggan sungguhan.
- [ ] **Belum**: topic restriction (membatasi Taqi AI agar hanya menjawab seputar jasa cetak/dokumen) — baru tertulis di bagian "Guardrail Tambahan" di atas, belum ada kode yang menegakkannya.
